const $=id=>document.getElementById(id);let meta,staffingConfig,scheduleData,vacationCapacityData,hiringRecommendations=[];
...
function yearEndHorizon(history){const start=new Date(`${history.last_date}T00:00:00`);start.setDate(start.getDate()+1);const end=new Date(start.getFullYear(),11,31);return Math.max(7,Math.round((end-start)/86400000)+1)}
function renderQueueCards(queues){
  const select=$('queueSort'),draw=()=>{
    const sorted=[...queues].sort(select.value==='coverage'?(a,b)=>a.capacity_coverage_percent-b.capacity_coverage_percent||b.demand-a.demand||a.name.localeCompare(b.name,'ru'):(a,b)=>b.demand-a.demand||a.capacity_coverage_percent-b.capacity_coverage_percent||a.name.localeCompare(b.name,'ru'));
    $('groupCards').innerHTML=sorted.map(queue=>{const coverage=queue.capacity_coverage_percent;return `<article class="group-card"><h3>${escapeHtml(queue.name)}</h3><div class="coverage"><strong>${fmt(coverage)}%</strong><span>прогнозное покрытие · цель SLA ${fmt(queue.sla_target_percent)}%</span></div><div class="bar"><i style="width:${Math.min(100,coverage)}%;background:${coverage<queue.sla_target_percent-10?'var(--red)':coverage<queue.sla_target_percent?'var(--amber)':'var(--green)'}"></i></div><div class="group-meta"><span><b>${fmt(queue.demand)}</b>прогноз</span><span><b>${fmt(queue.deficit)}</b>не покрыто</span><span><b>${fmt(queue.coverage_percent)}%</b>факт SLA</span></div></article>`}).join('');
  };
...
async function init(){
  try{
    ...
    staffingConfig={positions:meta.positions,operation_start_hour:meta.operation_start_hour,operation_end_hour:meta.operation_end_hour,sla_groups:meta.sla_groups,sla_queues:meta.sla_queues};renderPositions();renderSlaSettings();renderMethodCapacities();
    $('horizon').value=yearEndHorizon(meta.history);
    await analyze();
...
function render(data){
  ...
  const s=data.summary, hires=s.recommendations.reduce((a,r)=>a+r.people,0);hiringRecommendations=s.recommendations;
  ...
  $('changeKpiNote').textContent=change===0?`Ожидается столько же обращений, сколько за аналогичный период прошлого года ${shortDate(period.history_start)}–${shortDate(period.history_end)}`:`В период ${shortDate(period.forecast_start)}–${shortDate(period.forecast_end)} ожидается на ${fmt(Math.abs(change))}% ${direction} обращений, чем за аналогичный период прошлого года ${shortDate(period.history_start)}–${shortDate(period.history_end)}`;
  $('coverageKpi').textContent=`${fmt(s.sla.actual_percent)}%`;
  $('coverageKpi').style.color=s.sla.meets_target?'var(--green)':'var(--amber)';
  $('coverageKpiNote').textContent=`За ${shortDate(s.sla.period_start)}–${shortDate(s.sla.period_end)}: ${fmt(s.sla.sample_size)} обращений. Проверка по настроенным целям групп и очередей.`;
  ...
  renderQueueCards(s.queues);
  renderQuality(s.backtest);renderChart(data.daily);renderExpectedLoad(data.expected_load);renderRiskWidget(data.heatmap,s.queues);renderStaff(data.schedule);
}
function renderExpectedLoad(load){
  if(!load?.daily?.length)return;
  const select=$('loadDimension'),draw=()=>{
    const dimension=select.value,days=load.daily,keys=[...new Set(days.flatMap(day=>Object.keys(day[dimension]||{})))];
    const colors=['#287f6b','#4269c7','#a0662f','#8a55ad','#b34f65','#367f9e','#777a2f','#ce772c','#218c74','#ba4a00'];
    const max=Math.max(...days.map(day=>Object.values(day[dimension]||{}).reduce((a,b)=>a+b,0)),1);
    $('expectedLoadChart').innerHTML=days.map(day=>{const values=day[dimension]||{},total=Object.values(values).reduce((a,b)=>a+b,0),parts=keys.map((key,index)=>`<i style="height:${total?values[key]/max*100:0}%;background:${colors[index%colors.length]}" title="${escapeHtml(key)}: ${fmt(values[key]||0)}"></i>`).join('');const tip=`${date(day.date)} · всего ${fmt(total)} периодов · ${keys.map(key=>`${key}: ${fmt(values[key]||0)}`).join(' · ')}`;return `<div class="expected-load-day" title="${escapeHtml(tip)}"><small>${new Date(`${day.date}T00:00:00`).toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'})}</small><div class="expected-load-track">${parts}</div><b>${fmt(total)}</b></div>`}).join('');
    const months={};days.forEach(day=>{const month=day.date.slice(0,7);months[month]??={};Object.entries(day[dimension]||{}).forEach(([key,value])=>months[month][key]=(months[month][key]||0)+value)});$('expectedLoadMonths').innerHTML=Object.entries(months).map(([month,values])=>`<div class="expected-month"><b>${new Date(`${month}-01T00:00:00`).toLocaleDateString('ru-RU',{month:'long',year:'numeric'})}</b><strong>${fmt(Object.values(values).reduce((a,b)=>a+b,0))} <small>периодов</small></strong><span>${Object.entries(values).map(([key,value])=>`${escapeHtml(key)}: ${fmt(value)}`).join(' · ')}</span></div>`).join('');
  };select.onchange=draw;draw();
}
...
function renderChart(rows){
  const max=Math.max(...rows.map(r=>r.missing_people),1);
  $('dailyChart').innerHTML=rows.map((r,index)=>{
    const count=r.missing_people,uncovered=r.uncovered_groups.length>0;
    const rolesNeeded=Object.entries(r.missing_by_role).map(([role,n])=>`${role}: +${n}`).join(' · ');
    const groupsNeeded=Object.entries(r.deficit_by_group||{}).map(([group,n])=>`${roles[group]} — ${fmt(n)} обращ.`).join(' · ');
    const peakRoles=Object.entries(r.peak_missing_by_role||{}).map(([role,n])=>`${role}: ${n}`).join(' · ');
    const deficit=fmt(sum(r.deficit)),hours=fmt(r.missing_person_hours);
    const tip=count?`${date(r.date)}. Нужно добавить ${count} ${count===1?'человека':'человек'} на 8-часовую смену: ${rolesNeeded}. Куда: ${groupsNeeded}. Основание: ${deficit} непокрытых обращений = ${hours} человеко-часа дефицита, затем делим на 8 часов. Самый сложный час — ${String(r.peak_hour).padStart(2,'0')}:00: одновременно не хватало ${r.peak_missing_people} человек (${peakRoles}).`:`${date(r.date)}. Потребность покрыта, дополнительных сотрудников не нужно.`;
    const warning=uncovered?` · Нет подходящей должности для: ${r.uncovered_groups.map(g=>roles[g]).join(', ')}`:'';
    const edge=index<3?'left':index>=rows.length-3?'right':'center';
    return `<div class="people-day" tabindex="0" data-edge="${edge}" title="${escapeHtml(tip+warning)}" data-tip="${escapeHtml(tip+warning)}" aria-label="${escapeHtml(tip+warning)}"><b>${uncovered?'≥':''}${count}</b><div class="people-track"><i style="height:${count/max*100}%"></i></div><small>${new Date(`${r.date}T00:00:00`).toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'})}</small></div>`;
  }).join('');
  $('chartDates').textContent=rows.length?`${date(rows[0].date)} — ${date(rows.at(-1).date)}`:'';
}
function openHiringModal(){const content=$('hiringModalContent');content.innerHTML=hiringRecommendations.length?hiringRecommendations.map(item=>`<article class="hiring-plan-card"><div class="hiring-plan-head"><div><small>КУДА НАНИМАТЬ</small><h3>${escapeHtml(item.role)}</h3></div><strong>+${item.people}</strong></div><div class="hiring-plan-grid"><div><small>График</small><b>${escapeHtml(item.schedule_type||'5/2')} · ${item.shift_duration_hours||8} ч</b></div><div><small>Время смены</small><b>${String(item.shift_start??9).padStart(2,'0')}:00–${String(item.shift_end??17).padStart(2,'0')}:00</b></div><div><small>Пиковая просадка</small><b>${item.peak_hours?.length?item.peak_hours.map(hour=>`${String(hour).padStart(2,'0')}:00`).join(', '):'не выявлена'}</b></div><div><small>Основание</small><b>${item.sla_current!==undefined?`SLA ${fmt(item.sla_current)}% при цели ${fmt(item.sla_target)}%`:`дефицит ${fmt(item.deficit_periods||0)} обращений`}</b></div></div><p><b>Почему ${escapeHtml(item.schedule_type||'5/2')}:</b> ${escapeHtml(item.schedule_reason||'выбран по распределению почасового дефицита')}${item.minimum_rotation_people===2?' · минимум два сотрудника для чередования смен при устойчивой просадке':''}.</p></article>`).join(''):'<div class="ok-state">Дополнительный найм по текущему прогнозу не требуется.</div>';$('hiringModal').hidden=false;document.body.classList.add('modal-open');$('closeHiringModal').focus()}
function closeHiringModal(){$('hiringModal').hidden=true;document.body.classList.remove('modal-open');$('recommendationKpi').focus()}
...
function slaFields(scope,name,settings,inherit=false){return `<div class="sla-row" data-sla-scope="${scope}" data-sla-name="${escapeHtml(name)}"><b>${escapeHtml(scope==='group'?roles[name]:name)}</b><label>Цель, %<input data-sla-field="target_percent" type="number" min="1" max="100" step="0.1" value="${settings?.target_percent??''}" placeholder="${inherit?'Из группы':''}"></label><label>SLA, минут<input data-sla-field="threshold_minutes" type="number" min="1" max="1440" step="1" value="${settings?.threshold_minutes??''}" placeholder="${inherit?'Из группы':''}"></label></div>`}
function renderSlaSettings(){$('slaGroupsEditor').innerHTML=Object.keys(roles).map(group=>slaFields('group',group,staffingConfig.sla_groups[group])).join('');$('slaQueuesEditor').innerHTML=Object.entries(meta.queue_groups).flatMap(([group,queues])=>queues.map(queue=>slaFields('queue',queue,staffingConfig.sla_queues[queue],true))).join('')}
function readSlaSettings(){const groups={},queues={};document.querySelectorAll('[data-sla-scope]').forEach(row=>{const value={};row.querySelectorAll('[data-sla-field]').forEach(input=>value[input.dataset.slaField]=input.value===''?null:Number(input.value));if(row.dataset.slaScope==='group')groups[row.dataset.slaName]=value;else if(Object.values(value).some(item=>item!==null))queues[row.dataset.slaName]=value});return {sla_groups:groups,sla_queues:queues}}
...
document.querySelectorAll('.tab').forEach(tab=>tab.addEventListener('click',()=>switchTab(tab.dataset.tab)));document.querySelectorAll('[data-open-tool]').forEach(button=>button.addEventListener('click',()=>switchTab(button.dataset.openTool)));document.querySelectorAll('[data-hr-section]').forEach(button=>button.addEventListener('click',()=>switchHrSection(button.dataset.hrSection)));$('homeBtn').addEventListener('click',()=>switchTab('home'));$('employeeForm').addEventListener('submit',submitEmployee);$('hrDynamicForm').addEventListener('submit',submitHrRecord);$('toggleEmployeeRegistry').addEventListener('click',()=>{const registry=$('employeeRegistry');registry.hidden=!registry.hidden;$('toggleEmployeeRegistry').textContent=registry.hidden?'Показать реестр':'Скрыть реестр'});$('scheduleSheet').addEventListener('change',()=>{scheduleData=null;loadSchedule()});$('scheduleActivityFilter').addEventListener('change',renderSchedule);$('sheet').addEventListener('change',analyze);$('horizon').addEventListener('change',analyze);$('lookback').addEventListener('change',analyze);$('addPositionBtn').addEventListener('click',addPosition);$('savePositionsBtn').addEventListener('click',savePositions);$('uploadHistoryBtn').addEventListener('click',()=>$('historyFile').click());$('historyFile').addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;$('historyFileLabel').textContent=`${file.name} · ${fmt(file.size/1024/1024)} МБ`;uploadHistory(file)});$('uploadQueuesBtn').addEventListener('click',()=>$('queueFile').click());$('queueFile').addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;$('queueFileLabel').textContent=`${file.name} · ${fmt(file.size/1024/1024)} МБ`;uploadQueues(file)});$('reserve').addEventListener('input',e=>$('reserveValue').textContent=`${e.target.value}%`);$('runBtn').addEventListener('click',async()=>{await analyze();switchTab('forecast')});$('resetBtn').addEventListener('click',()=>{['sharedGrowth','specialistGrowth','expertGrowth'].forEach(id=>$(id).value=0);$('horizon').value=yearEndHorizon(meta.history);$('lookback').value=48;$('reserve').value=15;$('reserveValue').textContent='15%';$('vacancies').checked=false;$('sheet').value=meta.default_schedule_sheet;analyze()});window.addEventListener('popstate',()=>switchTab(sectionFromUrl(),false));init();
...
$('recommendationKpi').addEventListener('click',openHiringModal);
$('recommendationKpi').addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openHiringModal()}});
$('closeHiringModal').addEventListener('click',closeHiringModal);
$('hiringModal').addEventListener('click',event=>{if(event.target===$('hiringModal'))closeHiringModal()});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!$('hiringModal').hidden)closeHiringModal()});
