...
function renderQueueCards(queues){
  const select=$('queueSort'),draw=()=>{
    const sorted=[...queues].sort(select.value==='coverage'?(a,b)=>a.coverage_percent-b.coverage_percent||b.sla_sample_size-a.sla_sample_size||a.name.localeCompare(b.name,'ru'):(a,b)=>b.sla_sample_size-a.sla_sample_size||a.coverage_percent-b.coverage_percent||a.name.localeCompare(b.name,'ru'));
    $('groupCards').innerHTML=sorted.map(queue=>`<article class="group-card"><h3>${escapeHtml(queue.name)}</h3><div class="coverage"><strong>${fmt(queue.coverage_percent)}%</strong><span>цель ${fmt(queue.sla_target_percent)}% за ${fmt(queue.sla_threshold_minutes)} мин</span></div><div class="bar"><i style="width:${Math.min(100,queue.coverage_percent)}%;background:${queue.coverage_percent<queue.sla_target_percent-10?'var(--red)':queue.coverage_percent<queue.sla_target_percent?'var(--amber)':'var(--green)'}"></i></div><div class="group-meta"><span><b>${fmt(queue.sla_sample_size)}</b>обращений</span><span><b>${fmt(queue.outside_sla)}</b>вне SLA</span><span><b>${fmt(queue.demand)}</b>прогноз</span></div></article>`).join('');
  };
